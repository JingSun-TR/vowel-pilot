...
export default function SpectrumBars({ spectrum, voicing }) {
  if (!spectrum || spectrum.length === 0) return null
  const bars = spectrum.length, sr = 22050, binHz = sr / bars
  ...
  return (
    <div className="flex items-end gap-px h-12 w-full" style={{imageRendering:'pixelated'}}>
      {Array.from(spectrum).map((db,i)=>{
        const freq=i*binHz, h=Math.max(0,Math.min(1,(db-minDb)/(maxDb-minDb)))
        let color='#4b5563'
        ...
        return <div key={i} className="flex-1 rounded-t-sm transition-all duration-75"
          style={{height:`${h*100}%`,backgroundColor:color,opacity:voicing>0.15?0.4+h*0.6:0.15,minWidth:'1px'}} />
      })}
    </div>
  )
}
