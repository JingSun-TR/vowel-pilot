...
export default function VictoryScreen({ score, onRestart }) {
  const r = score>=4000?3:score>=2000?2:1
  return (
    <motion.div className="w-full h-full flex flex-col items-center justify-center overflow-hidden px-4"
      initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      {[...Array(20)].map((_,i)=>(
        <motion.div key={i} className="absolute w-1 h-1 rounded-full bg-amber-400"
          style={{left:`${Math.random()*100}%`,top:`${Math.random()*100}%`}}
          animate={{y:[0,-100],opacity:[0,1,0]}} transition={{duration:2+Math.random()*2,repeat:Infinity,delay:Math.random()*2}} />
      ))}
      <motion.div className="relative z-10 text-center" initial={{y:40,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:0.3}}>
        ...
        <div className="flex justify-center gap-2 mt-6">
          {[1,2,3].map(i=>(<motion.div key={i} initial={{scale:0,rotate:-90}} animate={{scale:1,rotate:0}} transition={{delay:0.8+i*0.2,type:'spring'}}>
            <Star className={`w-10 h-10 md:w-12 md:h-12 ${i<=r?'text-amber-400 fill-amber-400':'text-gray-700'}`} />
          </motion.div>))}
        </div>
        <motion.div className="mt-8 p-6 rounded-2xl bg-gray-900/60 border border-gray-700/50 max-w-xs mx-auto"
          ...
          <div className="flex items-center justify-center gap-1 mt-2 text-xs text-gray-500">
            <Sparkles className="w-3 h-3"/>{r===3?'パーフェクト！':r===2?'よくできました！':'もう一回挑戦！'}
          </div>
        </motion.div>
