...
export default function VictoryScreen({ score, onRestart }) {
  const maxScore = 10 * 500 // rough max
  const rating = score >= maxScore * 0.8 ? 3 : score >= maxScore * 0.5 ? 2 : 1
    ...
    >
      ...
      >
        ...
        <p className="text-lg text-gray-400 mt-2">
          All 10 vowel destinations reached
        </p>
        ...
        <div className="flex justify-center gap-2 mt-6">
          {[1, 2, 3].map(i => (
            ...
            >
              <Star
                className={`w-10 h-10 md:w-12 md:h-12 ${i <= rating ? 'text-amber-400 fill-amber-400' : 'text-gray-700'}`}
              />
            </motion.div>
          ))}
        </div>
        ...
        {/* Score */}
        <motion.div
          className="mt-8 p-6 rounded-2xl bg-gray-900/60 border border-gray-700/50 max-w-xs mx-auto"
