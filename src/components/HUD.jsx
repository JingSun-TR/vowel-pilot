...
export default function HUD({ level, totalLevels, score, timer, timeLimit, streak }) {
  const tl = Math.max(0, timeLimit - timer)
  return (
    <div className="px-4 pt-3 md:px-8 md:pt-4">
      <div className="max-w-xl mx-auto flex items-center justify-between gap-4">
        ...
        {streak>0&&<motion.div className="flex items-center gap-1" initial={{scale:0}} animate={{scale:1}} key={streak}>
          <Zap className="w-3.5 h-3.5 text-orange-400" /><span className="text-xs font-bold text-orange-300">x{streak}</span>
        </motion.div>}
        <div className={`flex items-center gap-1.5 ${tl<=5?'text-red-400':'text-gray-400'}`}>
          <Timer className="w-3.5 h-3.5" /><span className="text-sm font-mono">{tl}s</span>
        </div>
      </div>
    </div>
  )
